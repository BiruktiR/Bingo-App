...
import { TCartela, TFindCartelaSchema } from './cartela.schema';
...
export const findCartelaById = async (
  ...
  role: string,
  branchID: string,
  deletionValidation: boolean
) => {
  let data = await cartelaRepository
    .createQueryBuilder('cartelas')
    .leftJoinAndSelect('cartelas.branch', 'branch');
  if (deletionValidation) {
    data.leftJoinAndSelect('cartelas.game_cartelas', 'game_cartelas');
  }
  data.where('cartelas.id=:cartelaID', { cartelaID: id });
  if (role !== ROLES.superAdmin)
  ...
  return {
    ...
    ...(deletionValidation == true
      ? { game_cartelas: cartela.game_cartelas }
      : {}),
  };
};
...
export const updateCartela = async (
  ...
  return findCartelaById(id, role, branchID, false);
};

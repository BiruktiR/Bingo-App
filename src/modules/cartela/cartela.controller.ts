...
export const getById = expressAsyncHandler(
  async (req: Request, res: any, next: NextFunction) => {
    ...
    const cartela = await findCartelaById(id, role, user?.branch?.id, false);
    res.status(200).json({
      status: true,
...
export const deleteCartela = expressAsyncHandler(
  async (req: Request, res: any, next: NextFunction) => {
    ...
    const cartela = await findCartelaById(id, role, user?.branch?.id, true);
    if (cartela === null)
      return res.status(404).json({
        status: false,
        message: 'Cartela to be deleted is not found!',
      });
    if (cartela?.game_cartelas.length > 0)
      return res.status(302).json({
        status: false,
        message: 'Cartela cannot be deleted because it has relations',
      });
    await deleteById(id);
    res.status(200).json({
      status: true,
